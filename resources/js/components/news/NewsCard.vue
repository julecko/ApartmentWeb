...
const title = computed(() =>
  localeStore.locale === "sk" ? props.item.titleSk : props.item.titleEn
);
const summary = computed(() =>
  localeStore.locale === "sk" ? props.item.summarySk : props.item.summaryEn
);
const content = computed(() =>
  localeStore.locale === "sk" ? props.item.contentSk : props.item.contentEn
);
...
<template>
  <article :class="['card', { 'card--pinned': item.pinned }]">
    ...
    <button class="card__toggle" @click="toggle" :aria-expanded="expanded">
      ...
      <svg
        class="card__toggle-chevron"
        :class="{ 'card__toggle-chevron--up': expanded }"
        width="14"
        height="14"
        viewBox="0 0 14 14"
        fill="none"
